...
bot.start((ctx) => {
    ...
    ctx.reply(`
        Здравствуйте! Я бот N-KUDRINA. Я помогу вам узнать о наших курсах и записаться. Что вас интересует?
...
- Длительность: 1 день
- Стоимость в группе: 350,000 руб
- Стоимость индивидуального курса: 500,000 руб
...
Выберите курс, чтобы узнать больше или записаться.`,
        {
            reply_markup: {
                inline_keyboard: [
                    [
                        { text: 'Массаж-скульптор для лица “Серебряные аржентики”', callback_data: 'course_face' },
                        { text: 'Массаж-скульптор для тела “Серебряные аржентики”', callback_data: 'course_body' },
                    ],
                ],
            },
        }
...
Курс: ${user.course}`;
        ...
        ctx.reply('Спасибо! Ваши данные отправлены менеджеру. Мы скоро свяжемся с вами.');
    }
    if (user && user.step === 'callback') {
        // Находим номер телефона
        const phoneNumber = ctx.message.text;
        console.log(`Получен номер телефона от пользователя ${ctx.from.id}: ${phoneNumber}`);
        ...
        console.log(`Попытка отправки сообщения в группу менеджера: ${MANAGER_CHAT_ID}`);

        bot.telegram.sendMessage(MANAGER_CHAT_ID, messageToManager)
            .then(() => {
                console.log(`Сообщение отправлено в группу менеджера: ${MANAGER_CHAT_ID}`);
                ctx.reply('Спасибо! Мы получили ваш номер телефона. Наш менеджер свяжется с вами в ближайшее время.');
            })
            .catch((err) => {
    ...
    if (user && user.step === 'ask_question') {
        ...
        ctx.reply('Пожалуйста, отправьте ваш номер телефона, чтобы мы могли ответить на ваш вопрос.', phoneKeyboard);
    }
});
...
bot.on('contact', (ctx) => {
    ...
    if (user && user.step === 'ask_phone') {
        ...
        console.log(`Пользователь ${ctx.from.id} указал номер телефона: ${phoneNumber}`);
...
Вопрос: ${user.question}`;
        ...
        bot.telegram.sendMessage(MANAGER_CHAT_ID, messageToManager)
            ...
            .catch((err) => {
                console.error('Ошибка при отправке сообщения:', err);
                ctx.reply('Произошла ошибка при отправке вашего вопроса. Пожалуйста, попробуйте снова.');
            });
...
bot.action('callback', (ctx) => {
    ...
    console.log(`Сохранено состояние пользователя ${ctx.from.id}:`, userData[ctx.from.id]);
